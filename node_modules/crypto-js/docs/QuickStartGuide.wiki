...
PBKDF2 is a ***HIDDEN***-based key derivation function. In many applications of cryptography, user security is ultimately dependent on a ***HIDDEN***, and because a ***HIDDEN*** usually can't be used directly as a cryptographic key, some processing is required.

A salt provides a large set of keys for any given ***HIDDEN***, and an iteration count increases the cost of producing keys from a ***HIDDEN***, thereby also increasing the difficulty of attack.
