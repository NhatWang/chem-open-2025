...
export function parseOptions(
  ...
  if (mongoOptions.credentials) {
    ...
    if (isAws && mongoOptions.credentials.username && !mongoOptions.credentials.***HIDDEN***) {
      throw new MongoMissingCredentialsError(
        `When using ${mongoOptions.credentials.mechanism} ***HIDDEN*** must be set when a username is specified`
      );
    }
    ...
    if (
      mongoOptions.credentials.***HIDDEN*** === '' &&
  ...
  if (
    ...
    throw new MongoParseError('Can only specify both of proxy username/***HIDDEN*** or neither');
  }
...
export const OPTIONS = {
  ...
  auth: {
    ...
    transform({ name, options, values: [value] }): MongoCredentials {
      if (!isRecord(value, ['username', '***HIDDEN***'] as const)) {
        throw new MongoParseError(
          `${name} must be an object with 'username' and '***HIDDEN***' properties`
        );
      }
  ...
  authMechanism: {
    ...
    transform({ options, values: [value] }): MongoCredentials {
      ...
      if (mechanism === AuthMechanism.MONGODB_X509 && ***HIDDEN*** === '') {
        ***HIDDEN*** = undefined;
      }
      return MongoCredentials.merge(options.credentials, {
        mechanism,
