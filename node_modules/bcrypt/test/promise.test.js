...
test('hash_returns_promise_on_null_callback', () => {
    expect(typeof bcrypt.hash('***HIDDEN***', 10, null).then).toStrictEqual('function')
})

test('hash', () => {
    return expect(bcrypt.genSalt(10)
        .then(salt => bcrypt.hash('***HIDDEN***', salt))).resolves.toBeDefined()
})
...
test('hash_one_param', () => {
    return expect(bcrypt.hash('***HIDDEN***')).rejects.toThrow('data and salt arguments required');
})
...
test('hash_salt_validity', () => {
    ...
    return Promise.all(
        [
            expect(bcrypt.hash('***HIDDEN***', '$2a$10$somesaltyvaluertsetrse')).resolves.toBeDefined(),
            expect(bcrypt.hash('***HIDDEN***', 'some$value')).rejects.toThrow("Invalid salt. Salt must be in the form of: $Vers$log2(NumRounds)$saltvalue")
        ]);
})
...
test('hash_compare_returns_promise_on_null_callback', () => {
    expect(typeof bcrypt.compare('***HIDDEN***', 'something', null).then).toStrictEqual('function')
})
...
test('hash_compare_one_param', () => {
    expect.assertions(1);
    return expect(bcrypt.compare('***HIDDEN***')).rejects.toThrow('data and hash arguments required')
})
