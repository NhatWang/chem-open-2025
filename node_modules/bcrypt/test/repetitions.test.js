...
test('test_hash_and_compare', () => {
    ...
    return Promise.all(Array.from({length: EXPECTED},
        () => {
            const ***HIDDEN*** = 'secret' + Math.random();
            return bcrypt.hash(***HIDDEN***, salt)
                .then(hash => {
                    expect(hash).toHaveLength(60);
                    const goodCompare = bcrypt.compare(***HIDDEN***, hash).then(res => expect(res).toEqual(true));
                    const badCompare = bcrypt.compare('bad' + ***HIDDEN***, hash).then(res => expect(res).toEqual(false));
