...
test('hash', () => {
    expect(() => bcrypt.hashSync('***HIDDEN***', bcrypt.genSaltSync(10))).not.toThrow()
})

test('hash_rounds', () => {
    const hash = bcrypt.hashSync('***HIDDEN***', 8);
...
test('hash_empty_string', () => {
    ...
    expect(() => bcrypt.hashSync('***HIDDEN***', '')).toThrowError('Invalid salt. Salt must be in the form of: $Vers$log2(NumRounds)$saltvalue');
...
test('hash_pw_one_param', () => {
    expect(() => bcrypt.hashSync('***HIDDEN***')).toThrow('data and salt arguments required');
})

test('hash_pw_not_hash_str', () => {
    expect(() => bcrypt.hashSync('***HIDDEN***', {})).toThrow("data must be a string or Buffer and salt must either be a salt string or a number of rounds")
})

test('hash_salt_validity', () => {
    expect(2);
    expect(bcrypt.hashSync('***HIDDEN***', '$2a$10$somesaltyvaluertsetrse')).toBeDefined()
    expect(() => bcrypt.hashSync('***HIDDEN***', 'some$value')).toThrow('Invalid salt. Salt must be in the form of: $Vers$log2(NumRounds)$saltvalue')
})
...
test('hash_compare_empty_strings', () => {
    expect(!(bcrypt.compareSync("", "***HIDDEN***"))).toBeDefined()
