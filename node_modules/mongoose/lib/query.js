 ...
 *       ***HIDDEN***Hash: { type: String, select: false, required: true }
 *     });
 *     const UserModel = mongoose.model('User', userSchema);
 *
 *     const doc = await UserModel.findOne().orFail().schemaLevelProjections(false);
 *
 *     // Contains ***HIDDEN*** hash, because `schemaLevelProjections()` overrides `select: false`
 *     doc.***HIDDEN***Hash;
 *
 ...
 *     });
 *     const UserModel = mongoose.model('User', userSchema);
 *     const { _id } = await UserModel.create({ name: 'John', ***HIDDEN***: 'secret' })
 *
 ...
 *     let doc = await UserModel.findOne().select({ name: '$***HIDDEN***' });
 *     doc.name; // 'secret'
 *     doc.***HIDDEN***; // undefined
 *
 *     // With `sanitizeProjection`, Mongoose forces all projection values to be numbers
 *     doc = await UserModel.findOne().sanitizeProjection(true).select({ name: '$***HIDDEN***' });
 *     doc.name; // 'John'
 *     doc.***HIDDEN***; // undefined
 *
 *     // By default, Mongoose supports projecting in `***HIDDEN***` using `+***HIDDEN***`
 *     doc = await UserModel.findOne().select('+***HIDDEN***');
 *     doc.***HIDDEN***; // 'secret'
 *
 *     // With `sanitizeProjection`, Mongoose prevents projecting in `***HIDDEN***` and other
 *     // fields that have `select: false` in the schema.
 *     doc = await UserModel.findOne().sanitizeProjection(true).select('+***HIDDEN***');
 *     doc.***HIDDEN***; // undefined
 *
